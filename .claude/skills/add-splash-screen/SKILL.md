...
4. **Add `flutter_native_splash` to the app's `dev_dependencies`** (never
   `core`) plus the config block: `color`/`color_dark`, `image`, `web: true`,
   and the `android_12:` block with its own image + colours.
5. **Generate:** `flutter pub get` at the repo **root**, then from the app
   folder `dart run flutter_native_splash:create`.
6. **Walk the verification checklist in the guide below** — Android
   `launch_background.xml` + per-density `splash.png` + night variants +
   `values-v31`/`values-night-v31` styles + `android12splash.png`; iOS
   `LaunchImage` PNGs must be **real images, not 1×1 placeholders** (the #1
   silent failure), plus `LaunchBackground` + storyboard references; web
   `index.html` splash block. Report ✅/❌ per item.
7. **Test with a cold launch** on both platforms (kill the app first), in
   light and dark mode.
