---
name: add-storefront-template
description: >
  Add a new UI template (style pack) to apps/ecommerce/cordelia — the multi-tenant
  storefront — from a Figma kit read over MCP: sample the kit into a theme preset +
  pack constants + spec-sheet contracts, inventory its frames against the storefront
  surfaces cordelia already implements (discarding frames with no feature behind
  them), implement every surface end-to-end (shell, home, search, category/product
  details, cart, checkout + Razorpay, orders + track, address CRUD, profile + edit +
  change password, wishlist, notifications, privacy/terms) as
  presentation/templates/<id>/ slices over the unchanged shared layers, run the
  review + cross-pack reusability/promotion sweep, and update the spec sheet and
  progress docs. Proven exemplar: the dailymart port.
  Invoke with $add-storefront-template or ask to add a new storefront template/skin.
---

# add-storefront-template

Adds a **new UI template** to `apps/ecommerce/cordelia` (the multi-tenant
storefront) from a **Figma kit read over MCP** — a complete skin where every
storefront surface renders in the new pack over the unchanged shared
`domain`/`data`/`bloc` layers. `dailymart` is the proven exemplar. Follow every
phase below.
...
| Order rating (rate a **delivered** order) | `orders` / `OrdersEvent.rated` + `OrderReviewActions` | CTA on Track Order and/or the order card |
...
widget name across the workspace.

**When core is nearly right, name the missing parameter.** Almost every
pack-local fork here existed because the shared widget lacked exactly one knob
— a pinned-header mode, a subtitle line, an asset shorthand, a ripple, a text
alignment. Adding the parameter (plus its gallery variant) is right. **But if
absorbing your variant would need three or more new config params**, the fork
is correct: that turns the shared widget into a switchboard and every caller
into an override list. Write the reason in the file so the next review doesn't
undo it.
...
2. **Reusability sweep across all packs** — diff the new pack against the
   ...
   Promoting and **adopting** are one task in one commit: leaving the old
   copies in place keeps the duplication *and* adds a second thing to
   maintain. Run `flutter analyze` and the tests for the **whole workspace**,
   not just the app you touched — two apps here accumulated failing tests
   precisely because only the app under active work was ever run.

   **Copy audit before any string merge.** Two strings identical in the source
   ...
   test that catches the mistake. When the same concept has a copy per pack, a
   fix to one pack's wording is a bug report for the others.

   **Failure-path audit.** Grep the new pack for: text a machine wrote
