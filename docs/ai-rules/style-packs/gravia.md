...
| Product Details' "Select QTY" chips | One "Select {option}" row per option axis (Size, Colour, a pack size), `SelectorChip` per value; the price row, discount meta chip, "Only N left", the stepper ceiling and the bottom bar all follow the selected **variant** | The kit drew the chips as decoration and knew only pack sizes. The general product model (up to 3 option axes, per-variant price and stock, `ProductEntity.variants`) made them a real choice that travels into the cart line as a `variantId`. A value with no sellable unit given the other picks is struck through but still tappable — picking it re-resolves the other rows. An `attributes` spec list renders under Key information |
...
| Product Details' Ratings & Reviews block | A section between Key Info and Similar Products, in the same heading/divider rhythm: the average + `RatingStars` + per-star histogram, a `GraviaActionButton` opening the write-review sheet, then one row per review | The Gravia kit predates reviews and draws no frame for them, so the block is composed from recipes the pack already owns rather than invented chrome. Reviews are a real feature — see `feature/storefront/reviews/`. A product nobody has reviewed renders an `EmptyState`, never `0.0` stars |
