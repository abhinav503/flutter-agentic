...
All of the audit is now closed (2026-08-24): deletes move to `stores/{id}/trash` (restore from the console's "Recently deleted", the API `/trash`, or the MCP `restore_record`; TTL on `purgeAt` after 30 days); a raw PUT / `upsert_records` that would update half or more of a store with ≥20 records needs `confirm: true`; control characters are stripped from every text field; App Check ships unenforced; TTL policies exist on `rateLimits.expiresAt` and `trash.purgeAt`.

Minimum-version gate: `platform/appConfig` (`npm run app-config -- --android X.Y.Z --ios X.Y.Z`, empty = no gate) served by `GET /api/app-config`; the app's splash checks it and shows an "Update required" screen, failing open on any fetch/parse problem. Raise it before flipping App Check enforcement.
