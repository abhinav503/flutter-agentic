...
- [ ] `dailymart` remaining surfaces — the Wishlist tab renders a designed coming-soon state; Category Details and the Address Form still open the `gravia` screens; My Orders has no destination in this template (no Orders tab), so its Profile row says so
