...
- [ ] `dailymart` remaining surfaces — the Wishlist tab renders a designed coming-soon state; Category Details still opens the `gravia` screen; My Orders has no destination in this template (no Orders tab), so its Profile row says so
