...
- [ ] Remaining **placeholder content** (audited 2026-08-04 — everything else in the storefront now reads real Firestore data): Terms & Conditions is still lorem ipsum (the Privacy Policy beside it was rewritten); onboarding's three slides use seeded stock photos; grofast's address tiles cycle two bundled map images (nothing stores coordinates); and a product's photo "carousel" is single-slide because the catalog schema holds one image
