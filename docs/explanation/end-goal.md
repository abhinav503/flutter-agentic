...
> **Store publishing — deferred for `doc_scanner`, but now live for `cordelia`.** Shipping `doc_scanner` to the stores was never a phase goal (high friction, low payoff, BYOK review risk); its checklist stays in `docs/how-to/publish-to-stores.md`. **`cordelia` is a different case and is now on Google Play** — signed release bundle, package registered for Android developer verification, internal-testing track live and confirmed installable (2026-08-07, see `superapp-ecommerce-plan.md`). Production access still needs the Data safety form; a real live store is met (2026-08-11, and returns as a blocker if that store is ever unpublished) and the personal-account requirement of 12 testers is met (2026-08-19), leaving only the 14-day closed-testing window to elapse. **iOS followed on 2026-08-21** — signing, the APNs key, the `aps-environment` entitlement and the `remote-notification` background mode all landed, and `com.cordeliaapps.superapp` is delivered to App Store Connect and in **TestFlight**. Not yet submitted for review: the listing still needs screenshots, App Privacy labels, a privacy policy URL and a demo account, push is unverified on real hardware, and DSA trader status is unset so EU storefronts are excluded (see `superapp-ecommerce-plan.md`).
...
- [x] Checkout + per-store payments (Razorpay, and Stripe since 2026-08-10) — `CheckoutBloc` runs create-intent → native checkout → server-verified order write; each store settles into its **own** provider account (encrypted secret in `stores/{id}/private/payment`); provider-agnostic on both sides — the app routes through `PaymentGatewayRepository` and never names one, the server picks an adapter off the key prefix; test-mode payment-less path keeps checkout exercisable on web
...
- [x] `comingSoon` stubs cleared — the last of them, Login's social buttons, were removed for the iOS submission (Guideline 2.1 risk). `ValueConst.comingSoonMessage` now has no caller in feature code; the string and its translations survive for the next stub that needs one
- [ ] Remaining **placeholder content** (audited 2026-08-04, Terms closed 2026-08-06 — everything else in the storefront now reads real Firestore data): onboarding's three slides use seeded stock photos; grofast's address tiles cycle two bundled map images (nothing stores coordinates); and a product's photo "carousel" is single-slide because the catalog schema holds one image
...
- [x] Order rating — the shopper rates a **delivered** order 1–5 with optional text (server-gated to the order's own shopper and to `DELIVERED`, transactionally); offered from gravia's order card and every pack's Track Order, shown back on the card and in the admin console's order detail. The counterpart to product reviews, which are open to anyone signed in
- [x] gravia Track Order + routed My Orders — the last surfaces falling through to another pack's screens; dated status timeline, OTP, items, coupon-aware totals, copyable ids, and a Cancel/Rate CTA, composed from recipes the kit's own frames already established
- [x] In-app account deletion — the last row of every template's Profile (each on its own kit's delete glyph, behind that pack's confirm sheet, with a shared blocking overlay while the server works). Server-side it removes the shopper's profile, addresses, cart, favourites, recent searches and every review they wrote — each review through `deleteReview`, so the products they rated aren't left over-counted — then the Firebase Auth user last, so a partial failure stays recoverable. Orders are deliberately kept as the stores' sales records. Required by both app stores of any app offering sign-up (App Store Review Guideline 5.1.1(v))
- [x] Admin rebrand + catalog QoL (2026-08-05) — the console got a crawlable marketing landing page (SEO metadata + JSON-LD, brand palette promoted to `:root` so site and dashboard share one theme), and every catalog list gained client-side sorting (clickable column headers via `SortableTableHead`/`useTableSort` + `lib/sort.ts`; new Added and Valid-until columns backed by finally-surfaced `createdAtMs`) and filtering (Products: category/brand/stock; Coupons: status; SearchField added to Brands and Banners) — no query changes, no new indexes
- [x] "Generate sample data" seeder (2026-08-05) — one click writes a realistic Indian grocery catalog (~93 products, 10 Zepto-style categories, 32 real brands, 6 coupons, 4 banners) in a single atomic client-SDK `writeBatch`; product photos harvested live from Open Food Facts (CC-BY-SA, attributed in the dialog), landscape Unsplash banner photography, brand logos as verified Google favicons (10) + DiceBear monograms (22); `verify:seed-images` checks all 137 URLs and `patch-seed-images.mjs` retrofits already-seeded stores. Testing grofast against this real data forced three pack fixes (promo-card overflow → adaptive subtitle + budget rule fed into `/add-storefront-template`; carousel → full-bleed centred peeking like dailymart; category tiles → two-line centred names). BYOK LLM "custom store" generation deliberately deferred
- [x] Per-store notifications, end-to-end (2026-08-08) — the bundled per-template mock is gone. Sending is one server operation with two halves (the FCM push and the Firestore record), addressed by **topic** (`platform` / `store_{id}` / `user_{uid}`) so there is no token registry to fan out or clean up; the console composes both feeds with optional artwork, the storefront's notification centre merges store + platform + personal newest-first with server-side read receipts, and order transitions (placed / on the way / delivered / cancelled) send themselves in the **store's** language, since server-authored copy can't be localized by the app. A tap fetches the store behind the push, mounts its storefront and opens that store's notification centre. Android-complete; iOS push awaits an APNs key
- [x] Store publication lifecycle (2026-08-10) — `draft → pending → published` (plus `rejected`, and superadmin `unpublish`), every transition server-side because `firestore.rules` lets an owner write their own store doc. One dual-audience route owns the state machine; an owner-only readiness checklist (logo / ≥1 category / ≥1 product / payments connected) caches a `previewReady` flag discovery reads, so an owner previews their own half-built store in the real app and nobody else sees it. Stores predating the lifecycle read their old `"active"` marker as `published`
- [x] Stripe as a second payment provider (2026-08-10) — one interface, two adapters, the provider read off the key prefix so pasting a key *is* choosing one. Closes the gap where seven market catalogs existed but `quoteCart` hardcoded INR, so no EUR/GBP/USD store could take a payment. Stripe has no client signature (verification is a server-side re-fetch), which forced both halves of checkout onto one shared `checkout-quote.ts`
- [x] Delivery fee + per-store serviceability (2026-08-11) — the last open item from the original "Missing flows" list. A flat fee with an optional free-above threshold and postal-code **prefix** areas on the store doc; computed in one file so the payment intent and the order transaction can't disagree (a drift shows up as a failed payment verification, not a wrong label). Enforced at both checkout steps, pre-checked in the app for a localized refusal, rendered in every totals panel across three packs and in the console. Every store predating it reads back as free delivery everywhere, so no backfill
- [x] iOS — first App Store build (2026-08-21). Apple Developer enrolment, automatic signing under team `6VS9Z92P4Y`, the APNs auth key wired into Firebase Cloud Messaging, the **Push Notifications** capability + `remote-notification` background mode (two separate capabilities — Background Modes alone leaves no `aps-environment`, so no APNs token), export-compliance declared, iPad support dropped, and Login's coming-soon social buttons removed as a Guideline 2.1 risk. Delivered to App Store Connect and live in **TestFlight**; two delivery warnings traced to dependencies rather than app code and both fixed (`geolocator_apple` forcing `NSLocationAlwaysAndWhenInUseUsageDescription`, and Razorpay's dSYM shipping *inside* its vendored xcframework where Xcode never copies it). Not submitted for review: listing assets, App Privacy labels, a demo account, on-device push verification and DSA trader status all remain
- [x] Stock visibility (2026-08-21) — `stock` was enforced on both checkout writes and serialized to nobody, so a sold-out product browsed, carted and was refused at payment by an English server string. One serializer field now feeds every product payload; the app treats an absent key as "unknown, so sells" rather than zero; and all three packs mark a sold-out product and disable its add control, cap a quantity picker at what remains, flag an unbuyable cart line ("Out of Stock" / "Only 3 left") and refuse checkout in the shopper's own language before an address is picked. Closes the first of the go-live scan's minimum three; the support channel followed later the same day and browsing before signup on 2026-08-22, so all three are now shipped
- [x] Checkout's money gap + live stock (2026-08-21) — the follow-on to stock visibility. A payment taken for an order that then failed to write (the last unit going to someone else while the shopper paid) was left for the store owner to refund by hand; it now refunds itself at both failure points, records `stores/{id}/orphanedPayments/{paymentId}`, and proves the payment is the shopper's own first — an amount-free ownership check, since the earlier failure happens before verification can run. The refusal itself is now machine-readable (`code`/`available`/`refunded`) and rewritten in the shopper's language, the cart re-reads live stock on opening Cart, opening Checkout and after a refusal (with the save/fetch races closed), and stock under five reads "Only N left" on cards, product details and cart rows. The same pass closed a Razorpay amount hole it uncovered: the signature binds `orderId|paymentId` but never the amount, so a genuinely paid, never-placed order from an earlier cheaper cart verified against a larger one — Razorpay verification now re-reads the payment itself from the API, exactly as Stripe already did — which also exposed that a payment can be `authorized` (a hold nobody claimed) rather than `captured`, so the server now captures it after proving it is for this exact cart, instead of writing an order against money that would expire back to the shopper. Reservations — the hold that would remove the race rather than pay for it — remain designed and unbuilt
- [x] Support channel (2026-08-21) — the second of the go-live scan's minimum three. A store publishes its own contact (email, phone, opening hours) in **Settings → Store**; the app offers it from a **Help & Support** row in all three packs' Profile and from a "Need help with this order?" row in the body of every Track Order, with the order id, store, account and app version already written into the message. Publishing a support **email**, and a trading address (`Store.address`, required by the Store Profile form — seller-identity disclosure, which India's e-commerce rules and the EU DSA both want), are the fifth and sixth checks on the store readiness checklist, gating submit-for-review server-side beside "Payments connected" (not preview — an owner can still walk their own half-built store). `support@cordeliaapps.com` sits underneath for the problems a store cannot help with (signing in, the app, account deletion), and for stores published before the gate existed — the copy changes when a store publishes nothing, so the fallback is stated rather than silent. The `/refunds` policy is finally linked from the app it was written for. A `mailto:`/`tel:`, not a thread — and a launch that fails copies the address to the clipboard instead, so a device with no mail client is not a dead end
- [x] Alcohol and tobacco banned platform-wide (2026-08-22) — the App Store age rating is answered "None" for alcohol/tobacco references, which is only true while the catalogs are. One keyword list in six languages, with exempt phrases for the groceries that share a word (wine vinegar, root beer, cigarette lighter) and negators that clear a category when a product declares itself free of it; refused in the product form and on CSV import, and enforced **server-side** as a seventh store-readiness check, so no store can go live carrying any. A sweep of all 725 seed products across seven markets caught the three false positives the list would otherwise have shipped with (Britannia Bourbon the biscuit, Appy Fizz's "champagne-style bottle", Vinaigre de vin blanc). Written into `/terms` §6 and the owner docs
- [x] Reporting a review + blocking its author (2026-08-22) — the two halves of App Store Review Guideline 1.2 the app was missing. A report writes `…/reviews/{uid}/reports/{reporterUid}` and the review's `reportCount` in one transaction, keyed by the reporter so re-reporting edits rather than inflates; blocking is server-side under `users/{uid}/blockedShoppers`, and both review read paths take an optional token and drop that reader's blocked authors while leaving the rating summary alone. One sheet does both, because a reader who has had enough of someone shouldn't have to find a second control. The console sorts reported reviews first and gains **Keep** beside Delete
- [x] Browsing without an account (2026-08-22) — the last of the go-live scan's minimum three. Splash, onboarding, sign-out and expiry all land on Discovery; `SignInGateX.requireSignIn` *pushes* Login so returning is the navigator popping, and gates only what writes something of the shopper's (bag, wishlist, orders, profile, reviews). Five bugs it introduced were found and fixed in review — unhydrated cart on first add, a `ProfileState.signedOut` that had to exist or headers shimmered forever, an "Added to cart" that fired before the gate resolved, a blocked author reappearing in the details payload, and a push notification landing a guest on a 401 — plus one asymmetry it exposed: expiry cleared only the profile cache where sign-out cleared five things, and neither had ever cleared the selected delivery address
- [x] Ola Maps removed for the device's own geocoder (2026-08-22) — `OLA_MAPS_API_KEY` was never configured, so `/api/geo/reverse` and `/api/geo/autocomplete` had returned 502 since they shipped; Ola is also India-only against seven markets. `LocationService.currentPlace()` now resolves an address on the platform's own geocoder — no key, no quota, works everywhere, and needs no account, which is what lets a guest tap the storefront header's location row and see their postcode (permission asked on the tap, never at launch). Place autocomplete is dropped rather than replaced: no keyless equivalent exists. India Post's pincode lookup stays
- [ ] Firebase App Check — **before production**. The app's Firebase surface is
  Auth, Storage, Messaging and Crashlytics (no `cloud_firestore`; the storefront
  reads go through the Vercel API), so the real exposure is scripted signups and
  password-reset mail against a public API key, plus avatar uploads behind them.
  Register App Attest (iOS) + Play Integrity (Android), ship the SDK
  **unenforced** first, then enable enforcement once live traffic is attested —
  enforcing strands every installed build that lacks the SDK, which is why doing
  it while the install base is ~0 is nearly free. Debug tokens needed for local
  runs and simulators, which get no attestation.
- [ ] Delivery-agent apps — decided, not built (2026-08-11). Nothing downstream of "order placed" is done by the person doing it: a store admin advances each order from the console, and the handoff OTP is shown to shopper and admin while nothing verifies it. Agents get their own app, listed from a new entry point in Discovery's top-right corner — the one screen that exists outside any storefront, which is where an agent arrives too. Entry point reserved; the agent surface itself is unstarted
...
- [ ] Push notifications — FCM-backed `NotificationService` abstraction in `core/notifications/` *(capability shipped app-side in `doc_scanner` and now in `cordelia` against a real sending backend, plus the repeatable `add-notification-feature` skill; the `core` abstraction itself is still open — two app-side copies is the point at which it's worth extracting)*
