...
> **Store publishing — deferred for `doc_scanner`, but now live for `cordelia`.** Shipping `doc_scanner` to the stores was never a phase goal (high friction, low payoff, BYOK review risk); its checklist stays in `docs/how-to/publish-to-stores.md`. **`cordelia` is a different case and is now on Google Play** — signed release bundle, package registered for Android developer verification, internal-testing track live and confirmed installable (2026-08-07, see `superapp-ecommerce-plan.md`). Production access still needs a real live store, the Data safety form, and — on a personal developer account — 12 testers for 14 days.
