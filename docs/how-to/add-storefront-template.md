...
1. **Register the template.** Add the enum case + `wireValue` in
   ...
   **every** `StorefrontTemplateSwitch` in `app.dart` (grep for the type —
   ...
   exists to prevent). No bundled mock data: notifications became real
   per-store API data and `assets/data/templates/` was deleted with them.
...
3. Re-run Phase 4's suite after every change this sweep makes — a promotion
   into `core` is exactly the kind of edit that breaks a pack you did not
   open.
