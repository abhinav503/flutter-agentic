...
Read `docs/how-to/verify-payments.md` before shipping anything that touches checkout, refunds or stock — the release check for both payment providers: the two offline scripts (`verify:razorpay`, `verify:stripe`, plus `test:webhook`), the five device passes that scripts can't cover (ordinary order, wrong keys, sold-out mid-payment, held-card capture, cancel + refund), and the stock-display matrix to run across all three storefront templates.
...
Read `docs/how-to/add-storefront-template.md` (or run `/add-storefront-template`) when adding a new UI template (style pack / skin) to the `cordelia` multi-tenant storefront from a Figma kit — covers sampling the kit over Figma MCP into a theme preset + pack constants + spec-sheet contracts, inventorying kit frames against the storefront surfaces cordelia already implements (discarding frames with no feature behind them), implementing every surface end-to-end (shell, home, search, category/product details, cart, checkout + Razorpay, orders + track order, address CRUD, profile + edit + change password, wishlist, notifications, privacy/terms) as `presentation/templates/<id>/` slices over the unchanged shared layers, the cross-pack reusability/promotion review, and the spec-sheet + progress-doc updates. Proven exemplar: the `dailymart` port.
