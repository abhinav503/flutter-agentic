...
/// The Cordelia-brand glass header action — [AppIconButton]'s glass variant
/// with the SVG wiring and header-control sizing (45px disc, 20px icon)
/// baked in, so every hero-header control (back, search, favourite,
/// notification) is one line instead of a re-typed variant/size/iconBuilder
/// recipe that drifts (Home's discs had already drifted to a 25px icon —
/// kept via [iconSize]). App-level (not pack-level) because the shared auth
/// screens render it too — the `gravia` pack consumes it as
...
class CordeliaGlassIconButton extends StatelessWidget {
  ...
  static const double containerSize = CordeliaDimenConst.controlHeight;
