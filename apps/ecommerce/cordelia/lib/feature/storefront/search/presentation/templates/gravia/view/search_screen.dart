...
class _SearchScreenState extends BaseScreenState<SearchScreen> {
  ...
  void _addToCart(ProductEntity product, int quantity) =>
      showSnackBar(GraviaValueConst.addedToCartMessage(product.name, quantity));
  ...
  Widget _resultsBody(
    ...
    if (resultsError != null) {
      return ErrorView(
        message: GraviaValueConst.searchResultsErrorMessage,
    ...
    if (results.products.isEmpty && results.categories.isEmpty) {
      return EmptyState(
        ...
        title: GraviaValueConst.searchNoResultsTitle,
        subtitle: GraviaValueConst.searchNoResultsSubtitle(query),
      );
    }
