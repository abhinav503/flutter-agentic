...
void main() {
  ...
  group('band edges are per currency', () {
    ...
    test(
      'the western currencies use grocery-scale edges, not converted ones',
      () {
      ...
      test('${currency.wireValue}: every price lands in exactly one band', () {
        ...
        for (final price in probes) {
          ...
          expect(
            ...
            hasLength(1),
            reason:
                '$price matched ${matching.length} bands: '
  ...
  group('a label cannot disagree with its own predicate', () {
    ...
    test(
      'switching the store currency moves both the label and the filter',
      () {
